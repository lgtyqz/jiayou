...
OAuth tokens are stored only in sessionStorage and restored on reload while valid. localStorage remembers that Drive was connected, allowing one `prompt=none` reconnection attempt in a future browser session. If the Google browser session has ended, the app returns to local-only mode and asks the user to connect again. **Disconnect Drive** clears this remembered state without revoking the Google OAuth grant. A static GitHub Pages app cannot securely keep a client secret or provide server-managed refresh tokens, so indefinite unattended sign-in is not implemented.
