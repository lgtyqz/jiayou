...
- Drag cards to reorder, move between columns, or place in Priority. Drop a card on **Destroy** to delete it. Escape cancels a drag.
...
Every edit saves immediately to localStorage. While connected, Drive saves are debounced and serialized; local storage remains a fallback. A newer Drive board loads on connection. The board with the newer `updatedAt` wins; simultaneous edits from different devices are not merged. Keep one active editing tab/device to avoid conflicting writes.
...
The integration uses Google's OAuth redirect token flow with a one-time state check and the Drive REST API through native `fetch`; it does not load an external SDK or expose a client secret. A JSON board is saved in Drive's hidden application-data folder, accessible only to this app. **Saving…** and **Autosaved** disable the button during normal connected operation. Errors offer reconnection or a retry and preserve local edits.

OAuth tokens are stored in sessionStorage and restored on reload while valid. Google browser sign-in may persist across sessions, but Drive authorization expires and may require another click. A static GitHub Pages app cannot securely keep a client secret or provide server-managed refresh tokens; indefinite unattended sign-in is not implemented.
